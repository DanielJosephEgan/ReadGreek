...
const learnButton = document.querySelector('.test-button');

groupInputs.forEach((input) => input.addEventListener('change', () => {
  learnButton.disabled = ![...groupInputs].some((item) => item.checked);
...
learnButton.addEventListener('click', () => {
  ...
  const url = `soundboard.html?groups=${encodeURIComponent(selectedGroups.join(','))}`;
